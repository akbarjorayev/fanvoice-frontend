...
export function SendMessageForm({ creatorId, creatorName, creatorUsername, minPrice }: Props) {
  const t = useTranslations('sendMessageForm')
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (priceActual < effectiveMin) {
      setErrorMsg(t('priceTooLow', { amount: effectiveMinFormatted }))
      setStatus('error')
      return
    }

    if (priceActual > CREATOR_MAX_PRICE) {
      setErrorMsg(t('priceTooHigh', { amount: `${formatPrice(CREATOR_MAX_PRICE)} so'm` }))
      setStatus('error')
      return
    }
    ...
    return (
      <div className="animate-fade-up flex flex-col gap-6 py-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900 dark:text-white">{t('messageReady')}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">{t('payToDeliver', { name: creatorName })}</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 divide-y divide-gray-100 dark:divide-gray-800">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">{t('messageWritten')}</span>
          </div>
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <span className="text-sm font-medium text-gray-900 dark:text-white">
              {t('payToSend', { amount: priceFormatted })}
            </span>
          </div>
        </div>
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <textarea
          ...
          placeholder={t('messagePlaceholder', { name: creatorName })}
          rows={7}
          required
      ...
      <div className="space-y-2">
        ...
        <div
          ...
          className="flex items-center rounded-full border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900 overflow-hidden w-fit cursor-text focus-within:ring-2 focus-within:ring-violet-500/60 focus-within:border-transparent transition-all"
        >
          <input
            ref={priceRef}
            id="msg-price"
            ...
            required
            className="w-24 h-12 px-5 text-sm text-gray-900 dark:text-white bg-transparent focus:outline-none focus-visible:shadow-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
          />
        ...
        <p className="text-xs text-gray-400 dark:text-gray-600">
          {t('minimum', { amount: effectiveMinFormatted })}
        </p>
      </div>
